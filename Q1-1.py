...
def replaceData(numData): # numData	2차원 정수 배열
    ...
    for i in range(0,len(numData)):
        retData.append([])
        for j in range(0, len(numData[i])):
            if numData[i][j] <= 0:
                retData[i].append(0)
            elif numData[i][j] > 100:
                retData[i].append(numData[i][j] - (numData[i][j]//100 *100))
            else:
                retData[i].append(numData[i][j])
...
def getMaxSum(numData): # 요구 사항에 맞춰 처리된 2차원 정수 배열
    ...
    for i in range(0,4):
        for j in range(0, 4):
            sq = numData[i][j] + numData[i][j + 1] + numData[i + 1][j] + numData[i + 1][j + 1]
            if  sq > maxSum:
                maxSum = sq
        print(maxSum)
